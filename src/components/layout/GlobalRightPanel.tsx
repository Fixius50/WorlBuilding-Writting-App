...
const GlobalRightPanel: React.FC<GlobalRightPanelProps> = ({
  ...
  return (
    <aside
      className={`
        fixed top-0 right-0 h-full monolithic-panel border-y-0 border-r-0 shadow-2xl z-40 transition-all duration-500 ease-in-out flex flex-col rounded-none
        ${isOpen ? 'w-96 translate-x-0' : 'w-96 translate-x-full'}
      `}
    >
      {/* SHARP-EDGED RIGHT COLLAPSE BUTTON WITH BORDER-FIX */}
      <button
        onClick={onToggle}
        className={`
          absolute top-1/2 -translate-y-1/2 -left-10 w-10 h-24 
          bg-background border border-foreground/10 border-r-0
          rounded-none flex flex-col items-center justify-center gap-1
          transition-all duration-300 group
          hover:bg-indigo-500/10 hover:border-indigo-500/30
          ${isOpen ? 'text-indigo-500 shadow-[-4px_0_15px_-5px_rgba(99,102,241,0.3)] border-r-transparent' : 'text-foreground/60'}
        `}
        style={{
          borderRightColor: isOpen ? 'transparent' : undefined
        }}
        title={isOpen ? t('common.close_panel') : t('common.open_panel')}
      >
        <div className={`w-1 h-3 bg-current opacity-20 transition-all duration-500 ${isOpen ? 'h-6 opacity-40' : ''}`}></div>
      ...
      <div className="flex items-center border-b border-foreground/10 bg-foreground/[0.02]">
        <button
          ...
          className={`flex-1 py-4 flex flex-col items-center justify-center gap-1 text-[0.6rem] font-black uppercase tracking-widest transition-colors relative ${activeTab === 'CONTEXT' ? 'text-indigo-400' : 'text-foreground/60 hover:text-indigo-400'}`}
        ...
        <button
          ...
          className={`flex-1 py-4 flex flex-col items-center justify-center gap-1 text-[0.6rem] font-black uppercase tracking-widest transition-colors relative ${activeTab === 'QUICK_NOTES' ? 'text-indigo-400' : 'text-foreground/60 hover:text-indigo-400'}`}
        ...
        <button
          ...
          className={`flex-1 py-4 flex flex-col items-center justify-center gap-1 text-[0.6rem] font-black uppercase tracking-widest transition-colors relative ${activeTab === 'EXPLORER' ? 'text-indigo-400' : 'text-foreground/60 hover:text-indigo-400'}`}
      ...
      {title && activeTab === 'CONTEXT' && (
        <div className="px-6 py-3 border-b border-foreground/10 bg-white/[0.01]">
        ...
        <div className={`absolute inset-0 flex flex-col animate-in fade-in duration-300 ${activeTab === 'EXPLORER' ? 'z-10 opacity-100 pointer-events-auto' : 'z-0 opacity-0 pointer-events-none'}`}>
          <div className="p-4 border-b border-foreground/10">
            <div className="relative group mb-3">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-foreground/60 text-sm group-focus-within:text-indigo-400 transition-colors">search</span>
              <input
                type="text"
                placeholder={t('bible.search_entity')}
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full monolithic-panel rounded-none py-2 pl-9 pr-3 text-xs focus:border-indigo-500/50 outline-none transition-all text-foreground placeholder:text-foreground/60"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                <select
                  ...
                  className="appearance-none bg-background border border-foreground/10 rounded-none px-4 py-2 pr-10 text-xs font-bold text-foreground transition-all hover:bg-foreground/5 focus:border-indigo-500 outline-none w-full"
                >
                ...
                <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-xs text-foreground/60 pointer-events-none">expand_more</span>
              </div>
            </div>
          </div>

          <div className="p-4 border-b border-foreground/10 pt-3">
            <div className="flex items-center justify-between">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-foreground/60">{t('bible.explorer')}</h2>
              <button
                className="transition-colors hover:text-foreground text-indigo-400"
