...
const InteractiveMapView: React.FC<{ map: Entidad }> = ({ map }) => {
    const { t } = useLanguage();
    const [selectedMarker, setSelectedMarker] = useState<InteractionMarker | null>(null);
    ...
    const [connectionSource, setConnectionSource] = useState<InteractionMarker | null>(null);
    ...
    const saveMapAttributes = async (newAttributes: any) => {
        if (!localMap) return;
        try {
            const updatedMap = {
                ...localMap,
                    ...
                    <div className="p-6 h-full flex flex-col animate-in slide-in-from-right-4 duration-300">
                        ...
                        <div className="space-y-3 flex-1 overflow-y-auto">
                            ...
                            {connections.map((c: any, idx: number) => {
                                const s = markers.find((m: any) => m.id === c.sourceId);
                                const t = markers.find((m: any) => m.id === c.targetId);
                                return (
    ...
    const handleMarkerClick = (marker: InteractionMarker) => {
