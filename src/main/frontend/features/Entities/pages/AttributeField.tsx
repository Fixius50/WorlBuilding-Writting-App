...
const AttributeField: React.FC<AttributeFieldProps> = ({ 
        ...
        switch (plantilla.tipo) {
                ...
                return (
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className={`size-6 rounded-lg border flex items-center justify-center transition-all ${value === 'true' ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-glass-border text-transparent hover:border-indigo-500/50'}`}
                        >
                            <span className="material-symbols-outlined text-sm">check</span>
                        </button>
                        <span className="text-xs text-slate-500">{value === 'true' ? 'Enabled' : 'Disabled'}</span>
                    </div>
                );
            case 'select':
                let options: string[] = [];
                ...
                return (
                    <div className="relative">
                        <select
                            className="w-full bg-black/30 border border-white/5 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500/50 appearance-none cursor-pointer"
                            value={value || ''}
                            onChange={(e) => onChange(e.target.value)}
                        >
                            <option value="" className="bg-[#1a1a20] text-white">Select an option...</option>
                            {options.map((opt, i) => (
                                <option key={i} value={opt} className="bg-[#1a1a20] text-white p-2">
                                    {opt}
                                </option>
                            ))}
                        </select>
                ...
                return (
                    <div className="bg-black/20 border border-white/5 rounded-xl p-2 space-y-1 max-h-40 overflow-y-auto custom-scrollbar">
                        {multiOptions.map((opt, i) => (
                            ...
                            >
                                <div className={`size-4 rounded border flex items-center justify-center transition-all ${selectedValues.includes(opt) ? 'bg-indigo-500 border-indigo-500' : 'border-white/10 group-hover:border-white/30'}`}>
                ...
                return (
                    <div className="relative">
                        <select
                            ...
                            onChange={(e) => onChange(e.target.value)}
                        >
                            <option value="" className="bg-[#1a1a20] text-white">Select an Entity...</option>
                            {linkableEntities.map((ent) => (
                                <option key={ent.id} value={ent.id} className="bg-[#1a1a20] text-white p-2">
                                    {ent.nombre}
                                </option>
                            ))}
                        </select>
                ...
                return (
                    <div className="space-y-2">
                        <div className="flex gap-2">
                            <input
                                type="text"
                                className="flex-1 bg-black/30 border border-white/5 rounded-xl px-4 py-2 text-xs text-slate-500 focus:text-white focus:outline-none"
                        ...
                        {!value && (
                            <div className="w-full h-20 rounded-xl bg-white/5 border border-dashed border-white/10 flex items-center justify-center text-slate-600">
            ...
            case 'table':
                ...
                return (
                    <div className="space-y-2">
                        <textarea
                            className="w-full h-32 bg-black/30 border border-white/5 rounded-xl p-3 text-xs font-mono text-white/70 focus:outline-none focus:border-indigo-500/50"
                            value={tableData}
                            onChange={(e) => onChange(e.target.value)}
    ...
    return (
        <div className="p-4 border border-white/5 rounded-2xl bg-[#0d0d12]/50 backdrop-blur-sm relative overflow-hidden h-full flex flex-col group hover:border-indigo-500/20 transition-all">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-3 flex items-center justify-between gap-2">
