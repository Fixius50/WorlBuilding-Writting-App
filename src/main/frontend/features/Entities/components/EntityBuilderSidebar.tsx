...
const EntityBuilderSidebar: React.FC<EntityBuilderSidebarProps> = ({ templates, onAddTemplate, currentFields = [] }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#0a0a0c] border-l border-white/5 w-full animate-in slide-in-from-right duration-500">
            {/* Header */}
            <div className="p-6 border-b border-white/5 bg-gradient-to-br from-indigo-500/10 to-transparent">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                        ...
                        className={`size-8 rounded-full flex items-center justify-center transition-all ${isCreating ? 'bg-red-500 text-white rotate-45' : 'bg-white/5 text-indigo-400 hover:bg-indigo-500 hover:text-white'}`}
                    >
                ...
                {isCreating ? (
                    <div className="space-y-3 p-4 rounded-2xl bg-black/40 border border-indigo-500/20 animate-in fade-in zoom-in-95">
                        <input
                            autoFocus
                            placeholder="Nombre del atributo..."
                            className="w-full bg-transparent border-b border-white/10 py-1 text-xs text-white outline-none focus:border-indigo-400"
                            value={newTpl.nombre}
                            onChange={e => setNewTpl({ ...newTpl, nombre: e.target.value })}
                        />
                        <select
                            className="w-full bg-[#1a1a20] border border-white/10 rounded-lg px-2 py-1.5 text-[10px] text-white/70 outline-none"
                            value={newTpl.tipo}
                        ...
                        <button
                            ...
                            className="w-full py-2 bg-indigo-500 text-white rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-500/20"
                        >
                    ...
                    <div className="relative">
                        <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-600">search</span>
                        <input
                            type="text"
                            placeholder="Buscar plantillas..."
                            className="w-full bg-black/20 border border-white/5 rounded-xl pl-9 pr-4 py-2 text-xs text-white focus:border-indigo-500/50 outline-none transition-all placeholder:text-white/10"
                            value={filter}
                ...
                <div className="space-y-2 p-6 pt-0 mt-4">
                    ...
                    <div className="max-h-48 overflow-y-auto custom-scrollbar space-y-1 bg-white/[0.02] rounded-xl p-2 border border-white/5">
                        {currentFields.map((f, i) => (
                            <div key={f.id || i} className="flex flex-col p-1.5 rounded bg-white/5 border border-white/5">
                                ...
                                <span className="text-[10px] text-slate-400 truncate text-xs">
                ...
                {filteredTemplates.length === 0 ? (
                    ...
                    filteredTemplates.map(tpl => (
                        <div
                            ...
                            className="bg-[#1a1a20] hover:bg-white/5 border border-white/5 hover:border-indigo-500/30 rounded-xl p-4 cursor-grab active:cursor-grabbing transition-all group"
                        >
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-xs font-bold text-white group-hover:text-indigo-400 transition-colors">{tpl.nombre}</span>
                                <span className="material-symbols-outlined text-slate-600 text-sm opacity-50">drag_indicator</span>
                            </div>
