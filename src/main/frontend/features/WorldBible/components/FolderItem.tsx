...
interface FolderItemProps {
    ...
    onConfirmCreate: (tempId: any, name: string, type: 'folder' | 'entity', parentId: number, specialType?: string) => void;
    searchTerm: string;
    filterType: string;
    className?: string;
}
...
const FolderItem: React.FC<FolderItemProps> = ({
    ...
    return (
        <div className={`space-y-1 select-none ${className || ''}`}>
            <div
                className="flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-bold text-slate-400 hover:text-indigo-400 hover:bg-indigo-400/5 cursor-pointer transition-all group relative"
            ...
            >
                ...
                {isEditing ? (
                    <input
                        autoFocus
                        className="bg-white/5 border border-indigo-500/50 rounded px-2 py-0.5 text-xs text-white outline-none flex-1 min-w-0"
            ...
            {contextMenu && (
                <div className="fixed bg-[#0a0a0c] border border-white/5 shadow-2xl rounded-xl py-2 z-50 w-48 text-xs font-medium flex flex-col" style={{ top: contextMenu.y, left: contextMenu.x }} onClick={(e) => e.stopPropagation()}>
                    <div className="px-4 py-2 border-b border-white/5 font-bold text-slate-500 truncate">{contextMenu.name}</div>
            ...
            {isOpen && (
                <div className="ml-6 pl-4 border-l border-white/5 space-y-1">
                    {/* Render Subfolders */}
                    ...
                    {content.entities
                        ...
                        .map(ent => (
                            <Link
                                ...
                                className="flex items-center gap-3 px-3 py-2 rounded-xl text-[11px] font-medium text-slate-400 hover:text-indigo-400 hover:bg-indigo-400/5 transition-all group cursor-grab active:cursor-grabbing"
                                draggable
                                onDragStart={(e) => {
                    ...
                    {loaded && content.folders.length === 0 && content.entities.length === 0 && (
                        <div className="px-3 py-2 text-[10px] text-slate-500 uppercase font-black italic tracking-tighter">Sector Vacío</div>
                    )}
                </div>
            )}
        </div>
