...
const BibleCard = ({ item, type, linkTo, onContextMenu, onDelete }) => {
    ...
    return (
        <Link
            ...
            className="group relative flex flex-col p-4 rounded-3xl bg-surface-dark border border-glass-border hover:border-primary/50 hover:bg-white/5 transition-all duration-300 backdrop-blur-sm overflow-hidden"
        >
            ...
            <div className="relative z-10 flex items-start justify-between mb-4">
                <div className={`
                    ...
                    ${isFolder ? 'bg-primary/20 text-primary' : 'bg-white/5 text-text-muted group-hover:text-white backdrop-blur-md border border-white/5'}
                `}>
            ...
            <div className="relative z-10 mt-auto">
                <h3 className="text-sm font-bold text-white truncate group-hover:text-primary transition-colors drop-shadow-md">{item.nombre}</h3>
            ...
            <div className="absolute top-4 right-4 z-20 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <span className="material-symbols-outlined text-primary text-sm transform group-hover:translate-x-1 transition-transform drop-shadow-md">arrow_forward</span>
            </div>
        </Link>
    );
};
