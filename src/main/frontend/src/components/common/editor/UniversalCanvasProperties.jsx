...
const UniversalCanvasProperties = ({
    // Tool Props
    tool,
    setTool,
    strokeWidth,
    ...
    return (
        <div className="flex flex-col h-full gap-8 p-6 text-slate-300">
            <section className="bg-white/5 rounded-2xl p-4 grid grid-cols-4 gap-2">
                ...
                <button
                    onClick={onUndo}
                    className="aspect-square rounded-xl flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/10"
                    title="Undo"
                >
                ...
                <button
                    onClick={onClear}
                    className="aspect-square rounded-xl flex items-center justify-center text-slate-500 hover:text-red-400 hover:bg-red-400/10"
                    title="Clear Canvas"
                >
            ...
            <section className="space-y-6">
                    ...
                    <div className="flex bg-white/5 rounded-lg p-1">
                        {['butt', 'round', 'square'].map(cap => (
                            <button
                                ...
                                className={`flex-1 py-1 rounded text-[10px] uppercase transition-all ${lineCap === cap ? 'bg-primary text-white shadow-lg font-black' : 'hover:text-white text-slate-500'}`}
                            >
                ...
                <div className="space-y-3">
                    <span className="text-[10px] uppercase font-bold text-slate-500">Estilo de Trazo</span>
                    <div className="flex bg-white/5 rounded-lg p-1">
                        {[
                            ...
                            <button
                                ...
                                className={`flex-1 py-1.5 rounded flex items-center justify-center gap-2 text-[9px] uppercase transition-all ${strokeStyle === style.id ? 'bg-primary text-white shadow-lg font-black' : 'hover:text-white text-slate-500'}`}
                                title={style.label}
                            >
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1">
                                <button
                                    ...
                                    className={`text-slate-500 hover:text-white p-1 rounded hover:bg-white/5 ${layer.locked ? 'text-red-400 opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                                >
                                ...
                                <button
                                    ...
                                    className="p-1 text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 rounded hover:bg-white/5"
                                    title="Eliminar Capa"
                                >
