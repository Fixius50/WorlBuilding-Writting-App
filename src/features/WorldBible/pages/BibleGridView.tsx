...
const BibleGridView = () => {
 ...
 // Context Menu State
 const [contextMenu, setContextMenu] = useState<{ x: number, y: number, folder: Carpeta } | null>(null);

 const [renamingFolderId, setRenamingFolderId] = useState<string | number | null>(null);

 const [renameValue, setRenameValue] = useState('');

 const handleContextMenu = (e: React.MouseEvent, folder: Carpeta) => {
 e.preventDefault();
 e.stopPropagation();
 ...
 <h1 className="text-4xl font-black text-foreground tracking-tighter">{t('bible.root_index')}</h1>
 ...
 <button onClick={() => handleCreateSimpleFolder(null, 'FOLDER')} className="px-6 py-2 bg-primary hover:bg-primary-light text-foreground rounded-none text-xs font-bold flex items-center gap-2 transition-all shadow-lg shadow-primary/20">
 ...
 {folders.length === 0 && (
 <div className="p-20 text-center border-2 border-dashed border-foreground/10 rounded-[3rem] opacity-30 mt-8">
 <span className="material-symbols-outlined text-6xl mb-4 text-text-muted">library_books</span>
 <h3 className="text-xl font-bold text-foreground uppercase tracking-widest">{t('bible.empty_archive')}</h3>
 ...
 {contextMenu && (
 <div
 className="fixed monolithic-panel border border-foreground/10 shadow-2xl rounded-none py-2 z-50 w-52 text-xs font-medium flex flex-col animate-in fade-in zoom-in-95 duration-150"
 style={{ top: contextMenu.y, left: contextMenu.x }}
 onClick={(e) => e.stopPropagation()}
 >
 <div className="px-4 py-2 border-b border-foreground/10 font-bold text-text-muted truncate flex items-center gap-2">
 <span className="material-symbols-outlined text-sm text-primary">folder</span>
 {contextMenu.folder.nombre}
 </div>

 <button
 className="px-4 py-2.5 hover:bg-foreground/5 text-left flex items-center gap-3 text-foreground/80 hover:text-foreground transition-colors"
 onClick={() => {
