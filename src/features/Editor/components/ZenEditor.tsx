...
const ZenEditor: React.FC<ZenEditorProps> = ({ content, onUpdate, editable = true, paperMode = false }) => {
 ...
 editorProps: {
 attributes: {
 class: `prose ${paperMode ? 'prose-invert' : 'prose-invert'} max-w-none focus:outline-none min-h-[300px] p-8 ${paperMode ? 'text-foreground/60' : 'text-foreground/90'} leading-relaxed text-lg transition-all`,
