    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-[#0a0a0c] overflow-y-auto custom-scrollbar p-8">
            <header className="mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
                <h1 className="text-4xl font-black text-white tracking-tight mb-2 uppercase">Centro de Mando</h1>
                <p className="text-slate-500 font-serif italic text-lg opacity-80">Panorama analítico de tu universo creativo.</p>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
                ...
                <GlassPanel className="h-[400px] p-6 flex flex-col">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-6 px-2">Distribución de Entidades</h3>
                    <div className="flex-1 min-h-0">
                        {stats.entitiesByType.length > 0 ? (
                            ...
                            <div className="h-full flex items-center justify-center text-slate-600 text-xs italic">No hay datos suficientes</div>
                        )}
                    </div>
                </GlassPanel>
                ...
                <GlassPanel className="h-[400px] p-6 flex flex-col">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-6 px-2">Densidad Geográfica (Zonas)</h3>
                    <div className="flex-1 min-h-0">
                        <ResponsiveBar
                            data={[
                                { zone: 'Norte', count: 12 },
            ...
            <GlassPanel className="p-8">
                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-8">Cronología de Cambios Recientes</h3>
                <div className="space-y-4">
                    {stats.recentActivity.map((e, idx) => (
                        <div key={e.id} className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group">
                            <div className="flex items-center gap-4">
                                <div className="size-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 group-hover:scale-110 transition-transform">
                                    <span className="material-symbols-outlined text-sm">edit_document</span>
                                </div>
                                <div>
                                    <h4 className="text-sm font-bold text-white mb-0.5">{e.nombre}</h4>
                                    <p className="text-[10px] text-slate-500 font-medium">Modificado en {new Date(e.fecha_creacion).toLocaleDateString()}</p>
                                </div>
                            </div>
                    ...
                    {stats.recentActivity.length === 0 && <p className="text-xs text-slate-600 italic text-center py-8">Aún no hay actividad registrada.</p>}
                </div>
            </GlassPanel>
        </div>
    );
...
const StatCard: React.FC<{ title: string; value: number | string; icon: string; color: string }> = ({ title, value, icon, color }) => (
    <GlassPanel className="p-6 flex items-center gap-5 hover:border-white/10 transition-all cursor-default">
        <div className={`size-12 rounded-2xl bg-white/[0.03] flex items-center justify-center ${color} shadow-lg`}>
