...
const ArchitectLayout: React.FC = () => {
      ...
      const interval = setInterval(
        async () => {
          ...
          if (res.success) {
            ...
            useSettingsStore
              .getState()
              .addNotification("Error en copia automática", "error");
          }
        },
        5 * 60 * 1000,
      ); // 5 minutos
  ...
  // Panel derecho eliminado: dejamos API de contexto como no-op para no romper consumidores.
  const toggleRightPanel = useCallback(() => {
    // Antes: alternaba el panel derecho contextual.
  }, []);

  // CRUD Handlers
  const handleCreateSimpleFolder = useCallback(
    async (parentId: number | null = null, type: string = "FOLDER") => {
      if (projectId) {
        try {
          const folderName =
            type === "TIMELINE"
  ...
  const handleCreateQuickEntity = useCallback(
    async (parentId: number, name: string, type: string) => {
      if (projectId) {
        try {
  ...
  const handleDeleteFolder = useCallback(
    (folderId: number, parentId: number | null = null) => {
  ...
  const handleDeleteEntity = useCallback(
    (entityId: number, folderId: number) => {
  ...
  const handleCreateEntity = useCallback(
    (folderId: number | string, specialType: string = "entidadindividual") => {
  ...
  const sidebarClasses =
    panelMode === "floating"
      ...
      : `fixed top-0 left-0 h-full monolithic-panel border-y-0 border-l-0 shadow-2xl z-[200] ${panelMode === "binder" ? "" : "transition-all duration-500"} flex flex-col rounded-none ${leftOpen ? "w-64 translate-x-0" : "w-64 -translate-x-full"}`;
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-full h-full flex flex-col overflow-hidden">
            ...
            <div className="p-3 border-t bg-background mt-auto">
              <NavItem
                ...
                icon="analytics"
                label={t("project.analytics_title")}
                collapsed={false}
              />
              <NavItem
                to={`${baseUrl}/sync`}
        ...
        {panelMode === "floating" && (
          <div className="contents">
            ...
            {/* Panel derecho eliminado: se retiró el toggle flotante de propiedades. */}
          </div>
        )}
