...
const TopNavBar = () => {
  ...
  const activeProjectId = useMemo(() => {
    ...
    return routeProjectId || projectMatch?.[1] || entityMatch?.[1] || null;
  ...
  return (
    <div className="h-16 flex items-center justify-center pointer-events-none sticky top-0 z-50 px-8">
      <MonolithicPanel className="flex items-center gap-2 px-2 py-1.5 rounded-full pointer-events-auto monolithic-panel shadow-2xl">
        <Link
          to="/dashboard"
          className={`p-2 rounded-full transition-colors ${path === "/dashboard" ? "bg-primary text-primary-foreground shadow-lg" : "text-foreground/60 hover:text-foreground hover:bg-foreground/5"}`}
        >
        ...
        {path !== "/dashboard" && path !== "/settings" && activeProjectId && (
          <nav className="flex items-center gap-1">
            ...
            />
            <NavItem
              to={`/project/${activeProjectId}/writing`}
              icon="edit_note"
              label="Writing"
              active={path.includes("/writing")}
            />
            <NavItem
              to={`/project/${activeProjectId}/timeline`}
              icon="event_note"
              label="Timeline"
              active={path.includes("/timeline")}
            />
            <NavItem
              to={`/project/${activeProjectId}/map`}
              icon="map"
              label="Cartography"
              active={path.includes("/map")}
            />
            <NavItem
              to={`/project/${activeProjectId}/languages`}
              icon="translate"
              label="Linguistics"
              active={path.includes("/languages")}
            />
            <NavItem
              to={`/project/${activeProjectId}/graph`}
              icon="hub"
              label="Graph"
