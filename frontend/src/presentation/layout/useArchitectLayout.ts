...
import { Carpeta, Proyecto, FolderType } from '@domain/models/database';
...
export const useArchitectLayout = () => {
  ...
  const [activeModal, setActiveModal] = useState<'database' | 'notes' | 'stats' | 'sync' | null>(null);
  ...
  // Cargar estadísticas cuando el modal stats está activo
  useEffect(() => {
  ...
  // Actualizar hora en tiempo real cuando el modal stats está activo
  useEffect(() => {
  ...
  const getPageName = useCallback((): string => {
    ...
    switch (true) {
      ...
      case isTimeline:
        return t('nav.chronology') || 'Cronología';
      case isTime:
        return 'Calendarios';
      case isLanguages:
        return t('nav.languages') || 'Lingüística';
      case isPlanning:
  ...
  // CRUD Handlers
  const handleCreateSimpleFolder = useCallback(async (parentId: number | null = null, type: string = 'FOLDER'): Promise<void> => {
    switch (projectId !== null) {
      case true:
        if (projectId !== null) {
          try {
            ...
            const folderName = isTimelineFolder
              ? (t('timeline.title') || 'Nueva Dimensión')
  ...
  const handleCreateQuickEntity = useCallback(async (parentId: number, name: string, type: string): Promise<unknown> => {
    switch (projectId !== null) {
      case true:
        if (projectId !== null) {
          try {
  ...
  const handleRenameFolder = useCallback(async (folderId: number, newName: string): Promise<void> => {
    try {
      if (projectId !== null) {
  ...
  const handleDeleteFolder = useCallback((folderId: number, parentId: number | null = null): void => {
    setDeletionTarget({ id: folderId, type: 'folder', parentId });
    setConfirmOpen(true);
  }, []);

  const handleDeleteEntity = useCallback((entityId: number, folderId: number): void => {
  ...
  const handleCreateEntity = useCallback((folderId: number | string, specialType: string = 'entidadindividual'): void => {
