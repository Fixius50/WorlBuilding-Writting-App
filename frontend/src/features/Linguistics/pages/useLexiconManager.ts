...
export const useLexiconManager = (projectParam: string | undefined) => {
  ...
  const loadRules = useCallback(
    async (langId: number, allEntities: Entidad[]) => {
      const langRules: GrammarRule[] = allEntities
        .filter(
          (e) =>
            (e.tipo === "Rule" || e.tipo === "Regla") &&
            e.carpeta_id === langId,
        )
        .map((r) => {
  ...
  const loadLexicon = useCallback(
    async (langId: number, allEntities: Entidad[]) => {
      const langWords = allEntities
        .filter(
          (e) =>
            (e.tipo === "Word" || e.tipo === "Palabra") &&
            e.carpeta_id === langId,
        )
  ...
  const saveWord = async (
    updatedData: Word & { isNew?: boolean },
    editingWordId?: number | string,
  ) => {
