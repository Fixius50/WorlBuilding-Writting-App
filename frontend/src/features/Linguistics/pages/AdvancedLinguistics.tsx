      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <MonolithicPanel className="p-8 space-y-6">
            ...
            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-widest text-foreground/40">
                {activeTab === "conjugation"
                  ? "Regla de Sufijo (ej: -[e]s)"
                  : "Reglas de Cambio (Proximamente)"}
              </label>
              {activeTab === "conjugation" ? (
                <input
                  type="text"
                ...
                <div className="p-4 border border-dashed border-foreground/20 text-[10px] text-foreground/40 italic">
                  Las reglas de evolución (ej: p {">"} b) se configurarán en la
                  interfaz de matrices fonéticas.
                </div>
              )}
              <Button
                variant="primary"
          ...
          {result.length > 0 && (
            <div className="pt-8 border-t border-foreground/5">
              <label className="text-[10px] font-black uppercase tracking-widest text-foreground/40 block mb-4">
                Resultado Generado
              </label>
              <div className="flex flex-wrap gap-2">
                {result.map((r, i) => (
                  <span
                    key={i}
                    className="px-4 py-2 bg-primary/10 border border-primary/20 text-primary font-bold text-sm"
                  >
                    {r}
                  </span>
                ))}
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <MonolithicPanel className="p-6 border-l-4 border-emerald-500/50">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-foreground/60 mb-2">
              Tip de Conjugación
            </h4>
            <p className="text-[11px] text-foreground/40 leading-relaxed italic">
              Usa corchetes para opcionales:{" "}
              <code className="text-primary/60">-[e]r</code> añadirá 'er' si la
              palabra no termina en 'e', o solo 'r' si ya la tiene.
            </p>
          </MonolithicPanel>
          <MonolithicPanel className="p-6 border-l-4 border-amber-500/50">
            ...
            <p className="text-[11px] text-foreground/40 leading-relaxed italic">
              Esta herramienta permite proyectar cómo cambiarían tus palabras a
              lo largo de los siglos (Lenición, Palatalización, etc).
            </p>
          </MonolithicPanel>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-6 gap-px bg-foreground/10 border border-foreground/10">
            {[
              ...
              <div
                key={h}
                className="bg-background/80 p-2 text-[8px] font-black uppercase text-primary/60 text-center"
              >
                {h}
              </div>
            ))}
            {Array.from({ length: 18 }).map((_, i) => (
              <div
                key={i}
                className="bg-background/40 p-4 h-12 flex items-center justify-center text-xs font-serif text-foreground/20 hover:bg-primary/5 transition-colors cursor-crosshair"
              >
                ?
              </div>
            ))}
