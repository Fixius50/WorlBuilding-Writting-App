...
export const useLinguisticsHub = (projectParam?: string) => {
  ...
  const [wordToDelete, setWordToDelete] = useState<number | string | null>(
    null,
  );
  ...
  const [writingSystem, setWritingSystem] = useState<string>(
    WRITING_SYSTEM_TYPES.ALPHABET.id,
  );
  ...
  const handleSaveCurrentGlyph = useCallback(async () => {
    ...
    try {
      const wordToSave: Word & { isNew?: boolean } = {
        ...
        fecha_creacion:
          existingWord?.fecha_creacion || new Date().toISOString(),
  ...
  const handleSaveMeaning = useCallback(
    async (updatedData: Word & { isNew?: boolean }) => {
      try {
  ...
  const handleComposerChange = useCallback(
    (text: string) => {
      ...
      const glyphs = text.split("").map((char) => {
        const found = foundryGlyphs.find(
          (g) => g.lema.toLowerCase() === char.toLowerCase(),
        );
