...
const WritingView = () => {
    ...
    switch (!!notebook?.project_id) {
      case true:
        EntityUseCase.getAllByProject(notebook!.project_id).then((list) => {
          setEntities(list.filter(e => e.tipo !== 'Map' && e.tipo !== 'Mapa'));
        });
        break;
      default:
        break;
  ...
  useEffect(() => {
    const loadMeta = async () => {
      switch (!!currentPage?.id) {
        case true: {
          const saved = await WorkspaceUseCase.getSetting(`page_metadata_${currentPage.id}`);
          switch (!!saved) {
            case true: {
              try {
  ...
  const handleSaveMetadata = useCallback(async (newPov: string, newLoc: string, newDate: string) => {
    switch (!!currentPage?.id) {
      case true: {
        const meta = { povId: newPov, locationId: newLoc, sceneDate: newDate };
        await WorkspaceUseCase.saveSetting(`page_metadata_${currentPage.id}`, JSON.stringify(meta));
        break;
      }
      default:
        break;
      ...
      <div className="flex flex-col h-full monolithic-panel/95 select-none">
        ...
        <div className="flex border-b border-foreground/10 bg-background/40">
          ...
          >
            ...
            <span className="text-[9px] font-black uppercase tracking-wider hidden xl:inline">Archivador</span>
          </button>
          ...
          >
            ...
            <span className="text-[9px] font-black uppercase tracking-wider hidden xl:inline">Notas</span>
          </button>
          ...
          >
            ...
            <span className="text-[9px] font-black uppercase tracking-wider hidden xl:inline">Biblia</span>
          </button>
          ...
          >
            ...
            <span className="text-[9px] font-black uppercase tracking-wider hidden xl:inline">Escena</span>
          </button>
        </div>

        {/* Contenido Dinámico según la pestaña activa */}
        <div className="flex-1 overflow-hidden flex flex-col relative" style={{ backgroundColor: 'hsl(var(--background))' }}>
          {(() => {
            switch (panelTab) {
              case 'notes':
                return (
                  ...
                  <div className="flex-1 flex flex-col h-full overflow-hidden select-text">
                    <div className="p-4 border-b border-foreground/5">
                      <div className="relative">
                        <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-sm text-foreground/40 font-black">search</span>
                        <input 
                          type="text"
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                      {filteredEntities.length === 0 ? (
                        ...
                        filteredEntities.map((ent) => {
                          ...
                          let entDesc = '';
                          try {
                            const attrs = typeof ent.contenido_json === 'string'
                              ? JSON.parse(ent.contenido_json)
                              : (ent.contenido_json || {});
                            entImg = attrs.imageUrl || attrs.image || attrs.avatar || '';
                            entDesc = ent.descripcion || attrs.description || 'Sin descripción adicional en la biblia.';
                          } catch {}
                              ...
                              >
                                <div className="size-8 bg-primary/10 border border-primary/20 flex items-center justify-center text-primary text-xs shrink-0">
                                  <span className="material-symbols-outlined text-sm">
                                    {ent.tipo?.toUpperCase() === 'PERSONAJE' || ent.tipo?.toUpperCase() === 'INDIVIDUAL' ? 'person' : 
                                     ent.tipo?.toUpperCase() === 'LUGAR' || ent.tipo?.toUpperCase() === 'TERRITORIO' ? 'location_on' : 'category'}
                                  </span>
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="text-[11px] font-bold text-foreground truncate">{ent.nombre}</div>
                                  <div className="text-[8px] text-foreground/40 uppercase font-black tracking-widest">{ent.tipo}</div>
                                </div>
                                <span className="material-symbols-outlined text-sm text-foreground/20 transition-transform duration-300" style={{ transform: isExpanded ? 'rotate(90deg)' : 'rotate(0deg)' }}>
                                  chevron_right
                                </span>
                              </div>
                                ...
                                <div className="pt-2 border-t border-foreground/5 space-y-3 animate-in fade-in duration-300">
                                  {entImg && (
                                    <div className="aspect-video w-full overflow-hidden border border-foreground/10">
                                      <img src={entImg} alt={ent.nombre} className="w-full h-full object-cover" />
                                    </div>
                                  )}
                    ...
                    {currentPage ? (
                      <>
                        <div className="space-y-1">
                          <span className="text-[8px] font-black uppercase tracking-[0.2em] text-primary">Detalles Contextuales</span>
                          <h4 className="text-xs font-black uppercase text-foreground">Metadatos de Escena</h4>
                        </div>

                        {/* POV Character */}
                        <div className="space-y-2">
                          <label className="text-[9px] font-black uppercase tracking-widest text-foreground/40 px-0.5">Personaje POV (Punto de Vista)</label>
                          <select
                            value={povId}
                            onChange={(e) => {
                              setPovId(e.target.value);
                              handleSaveMetadata(e.target.value, locationId, sceneDate);
                            }}
                          ...
                          >
                            <option value="" className="text-foreground/40">Seleccionar personaje...</option>
                            {characters.map(char => (
                              <option key={char.id} value={char.id}>{char.nombre}</option>
                            ))}
                          </select>
                        </div>
                        ...
                        {/* Location */}
                        <div className="space-y-2">
                          <label className="text-[9px] font-black uppercase tracking-widest text-foreground/40 px-0.5">Escenario / Ubicación</label>
                          <select
                            value={locationId}
                            onChange={(e) => {
                              setLocationId(e.target.value);
                              handleSaveMetadata(povId, e.target.value, sceneDate);
                            }}
                            className="w-full bg-[#1e1e1e] border border-foreground/10 py-2.5 px-3 text-[10px] font-mono outline-none focus:border-primary/50 transition-all text-white"
                          >
                            <option value="" className="text-foreground/40">Seleccionar ubicación...</option>
                            {locations.map(loc => (
                        ...
                        {/* Simulated Date */}
                        <div className="space-y-2">
                          <label className="text-[9px] font-black uppercase tracking-widest text-foreground/40 px-0.5">Fecha del Acontecimiento</label>
                          <input
                            type="text"
                            ...
                            onChange={(e) => {
                              ...
                              handleSaveMetadata(povId, locationId, e.target.value);
                            }}
                        ...
                        <div className="pt-4 border-t border-foreground/5">
                          <p className="text-[9px] text-foreground/30 italic leading-relaxed">
                            Tip: Estos metadatos te ayudan a organizar cronológicamente tus escenas y a realizar un seguimiento de dónde se encuentran tus personajes en cada capítulo.
                          </p>
                        </div>
                      </>
                    ) : (
