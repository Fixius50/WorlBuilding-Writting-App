      ...
      <div className="w-[350px] monolithic-panel p-5 border border-[hsl(var(--panel-border))] bg-[hsl(var(--background)/0.98)] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] group-hover/card:border-[hsl(var(--primary)/0.5)] cursor-pointer">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={(e) => { e.stopPropagation(); onEditStart(event); }} className="size-6 flex items-center justify-center hover:bg-[hsl(var(--foreground)/0.05)]"><span className="material-symbols-outlined text-sm">edit</span></button>
              <button onClick={(e) => { e.stopPropagation(); onDeleteRequest(event.id); }} className="size-6 flex items-center justify-center hover:bg-rose-500/20 hover:text-rose-500"><span className="material-symbols-outlined text-sm">delete</span></button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
             ...
             <button 
               onClick={(e) => { e.stopPropagation(); onLinkRequest(event.id); }}
