...
const TimelineImportModal: React.FC<TimelineImportModalProps> = ({
  ...
  const { t } = useLanguage();
  const { handleImportSelect } = useTimelineImportModal(onImport, onClose);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 animate-in fade-in duration-300">
      <div className="w-full max-w-lg monolithic-panel p-12 bg-[hsl(var(--background))] border border-[hsl(var(--panel-border))] relative overflow-hidden shadow-2xl">
        ...
        <div className="mb-10 text-center">
          <h2 className="text-3xl font-black uppercase tracking-tighter text-[hsl(var(--foreground))] mb-3">
            Importar Timeline
          </h2>
        </div>
        <div className="max-h-[350px] overflow-y-auto pr-2 space-y-3 custom-scrollbar">
          {availableDimensions.map((dim) => (
            ...
            >
              <div className="text-left">
                ...
                <div className="text-[9px] text-[hsl(var(--foreground)/0.3)] uppercase tracking-widest">
                  Timeline
                </div>
              </div>
              <span className="material-symbols-outlined text-sm text-[hsl(var(--primary))] opacity-0 group-hover:opacity-100">
                add_circle
              </span>
            </button>
          ))}
          {availableDimensions.length === 0 && (
            <div className="py-10 text-center opacity-30 italic text-xs">
              No hay timelines disponibles para importar.
            </div>
          )}
        </div>
