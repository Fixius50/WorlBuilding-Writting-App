    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/60 animate-in fade-in duration-300">
      <div className="w-full max-w-xl monolithic-panel p-10 bg-[hsl(var(--background))] border border-[hsl(var(--panel-border))] relative overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-black uppercase tracking-tighter text-[hsl(var(--foreground))]">
              {t("timeline.link_entity")}
            </h2>
            <p className="text-[10px] text-[hsl(var(--foreground)/0.3)] uppercase tracking-widest mt-2">
              Vincular personaje o lugar
            </p>
          </div>
          <button
            onClick={onClose}
            className="size-10 flex items-center justify-center bg-[hsl(var(--foreground)/0.05)] hover:bg-destructive hover:text-primary-foreground transition-all shadow-inner"
          >
            ...
            >
              <div className="flex items-center gap-5">
                ...
                <div className="text-left">
                  <div className="text-[13px] font-black text-[hsl(var(--foreground))]">
                    {ent.nombre}
                  </div>
                  <div className="text-[9px] text-[hsl(var(--foreground)/0.3)] uppercase tracking-widest font-bold">
                    {ent.tipo}
                  </div>
                </div>
              </div>
              <span className="material-symbols-outlined text-sm text-[hsl(var(--primary))] opacity-0 group-hover:opacity-100 transition-opacity">
                add_link
              </span>
            </button>
          ))}
        ...
        <div className="mt-10 pt-8 border-t border-[hsl(var(--divider-border))] text-right">
          <button
            onClick={onClose}
            className="px-8 py-3 bg-[hsl(var(--foreground)/0.02)] text-[10px] font-black uppercase tracking-widest text-[hsl(var(--foreground)/0.3)] hover:text-[hsl(var(--foreground))] transition-colors"
          >
            {t("common.cancel")}
          </button>
        </div>
