...
const InteractiveMapView: React.FC<{
  ...
  const [newLevelPosition, setNewLevelPosition] = React.useState<
    "above" | "below"
  >("above");
              ...
              <div className="absolute inset-0 w-full h-full pointer-events-none select-none">
                {levelBgImages[activeLevelId] || mapImage ? (
                  ...
                  <div className="w-full h-full border border-foreground/10 flex items-center justify-center">
                    <svg
                      viewBox="0 0 800 600"
                      className="w-full h-full stroke-foreground/10 fill-none"
                    >
                      <path
                      ...
                      <text
                        x="420"
                        y="280"
                        className="fill-foreground/20 font-serif text-[1.8rem] italic tracking-wider"
                      >
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-4">
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                ...
                                onMouseEnter={() =>
                                  setHoveredLevelOpacityId(lvl.id)
                                }
                                onMouseLeave={() =>
                                  setHoveredLevelOpacityId(null)
                                }
                              >
                                <span className="material-symbols-outlined text-[16px]">
                                  opacity
                              ...
                              {editingLevelId === lvl.id ? (
                                <input
                                  ...
                                  onChange={(e) =>
                                    setLevelInputText(e.target.value)
                                  }
                                  onBlur={() => {
                    ...
                    <>
                      <div className="space-y-3">
                        {annotations.length === 0 ? (
                          <p className="font-serif text-[12px] italic text-foreground/30 text-center py-4">
                            Sin anotaciones en el Atlas.
                          </p>
                        ) : (
                          annotations.map((ann) => {
                            const linkedLevel = levels.find(
                              (l) => l.id === ann.levelId,
                            );
                            const isCurrentLevel =
                              activeLevelId === ann.levelId;
                            ...
                            return (
                              <div
                                key={ann.id}
                                className={`group border p-3 rounded relative ${annotationCardClass}`}
                              >
                      ...
                      <div className="flex flex-col gap-2 pt-2">
                        ...
                        <div className="flex items-center justify-between text-xs px-1">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setNewAnnotationLevelId(e.target.value)
                            }
                  ...
                  {activeSidebarTab === "info" && (
                    <div className="space-y-4">
                      <div>
                        <span className="font-mono text-[9px] tracking-[0.25em] text-foreground/45 uppercase block mb-1">
                          Atlas Activo
                        </span>
                        <h2 className="font-serif text-[1.4rem] text-foreground font-semibold leading-tight">
                          {map.nombre || "Supermapa"}
                        </h2>
                        <p className="font-sans text-[11px] text-foreground/45 mt-1">
                          {map.descripcion || "Visor de cotas de altitud"}
                        </p>
                      </div>
                      <div className="pt-4 border-t border-foreground/10 space-y-3 bg-transparent">
                        <span className="font-sans text-[11px] font-bold text-foreground/45 block">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="bg-foreground/[0.02] border border-foreground/5 p-3 rounded-lg text-center">
                            ...
                            <span className="text-[9px] uppercase tracking-wider text-foreground/40 block mt-0.5">
                              Niveles
                            </span>
                          </div>
                          <div className="bg-foreground/[0.02] border border-foreground/5 p-3 rounded-lg text-center">
            ...
            <div className="w-[48px] bg-background border border-foreground/15 shadow-2xl rounded-xl flex flex-col items-center py-4 gap-4 pointer-events-auto text-foreground">
              ...
              <div className="relative group flex items-center justify-center">
                <button
                  onClick={() =>
                    setActiveSidebarTab(
                      activeSidebarTab === "levels" ? null : "levels",
                    )
                  }
              ...
              <div className="relative group flex items-center justify-center">
                <button
                  onClick={() =>
                    setActiveSidebarTab(
                      activeSidebarTab === "info" ? null : "info",
                    )
                  }
        ...
        {viewMode === "2D" && (
          <>
            ...
            <div className="flex items-center gap-1.5 bg-foreground/[0.03] border border-foreground/10 px-3 py-1 rounded-full">
              <span className="material-symbols-outlined text-[13px] text-foreground/50">
                layers
              </span>
              <select
                value={activeLevelId}
              ...
              >
                {levels.map((l) => (
                  <option
                    key={l.id}
                    value={l.id}
                    className="bg-background text-foreground text-[10px] font-bold uppercase tracking-wider"
                  >
                    {l.name.split(":")[0]}
                  </option>
                ))}
        ...
        {viewMode === "2D" && appMode === "VIEW" && (
          <>
            ...
            <button
              ...
              title={
                overlayAllLayers
                  ? "Ver solo la capa seleccionada"
                  : "Superponer todas las capas del Atlas"
              }
            >
              <span className="material-symbols-outlined text-[13px]">
                {overlayAllLayers ? "sheets" : "layers_clear"}
              </span>
        ...
        {viewMode === "2D" && appMode === "EDIT" && (
          <>
            ...
            <div className="flex items-center gap-1 relative">
              ...
              <button
                onClick={() =>
                  setActiveMenu(activeMenu === "brush" ? null : "brush")
                }
              ...
              {activeMenu === "brush" && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-56 bg-background border border-foreground/15 p-4 rounded-xl shadow-2xl z-50 speech-bubble text-foreground animate-in fade-in slide-in-from-top-2 duration-200">
                  <span className="font-sans text-[10px] font-bold tracking-wider uppercase text-foreground/40 block mb-2">
                    Herramienta
                  </span>
                  ...
                  <span className="font-sans text-[10px] font-bold tracking-wider uppercase text-foreground/40 block mb-1">
                    Color de tinta
                  </span>
                  ...
                  <span className="font-sans text-[10px] font-bold tracking-wider uppercase text-foreground/40 block mb-1">
                    Opacidad calco: {Math.round(backdropOpacity * 100)}%
                  </span>
                  <input
                    type="range"
                    min="0"
