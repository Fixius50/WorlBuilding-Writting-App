...
export const useInteractiveMapView = (map: Entidad, onBack?: () => void) => {
  ...
  >({});
  const [levelBgImages, setLevelBgImages] = useState<
    Record<string, string | null>
  >({});
  ...
  const [annotationInputLevelId, setAnnotationInputLevelId] =
    useState<string>("");
  const [newAnnotationText, setNewAnnotationText] = useState<string>("");
  const [newAnnotationLevelId, setNewAnnotationLevelId] =
    useState<string>("l2");
  ...
  const loadMarkerCharacters = useCallback(
    async (entityId: number) => {
      try {
        const rels =
          await RelationshipUseCase.getRelationshipsByEntity(entityId);
  ...
  useEffect(() => {
    if (map?.contenido_json) {
      latestContenidoJsonRef.current =
        typeof map.contenido_json === "string"
          ? map.contenido_json
  ...
  const handleAddLevel = useCallback(
    (name: string, position: "above" | "below" = "above") => {
  ...
  const handleUploadLevelBgImage = useCallback(
    (levelId: string, fileDataUrl: string) => {
  ...
  const handleSaveEditLevel = useCallback(
  ...
  const handleSaveEditAnnotation = useCallback(
    (id: string, newText: string, levelId: string) => {
