...
const CreateArchetypeModal: React.FC<CreateArchetypeModalProps> = ({
  isOpen,
  onClose,
  onCreate,
  parentFolder,
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 p-4">
      <div className="bg-background w-full max-w-5xl rounded-none shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-foreground/10 overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8 overflow-y-auto flex-1 no-scrollbar">
          <div
            className={`grid grid-cols-1 ${!isRoot ? "lg:grid-cols-[1fr_1.8fr]" : ""} gap-12`}
          >
                          ...
                          {group.ids.map((id) => {
                            ...
                            return (
                              <div
                                key={type.id}
                                onClick={() =>
                                  setTipo(type.id as HierarchyTypeId)
                                }
                              ...
                              >
                                <div
                                  className={`size-10 flex items-center justify-center bg-background border border-foreground/5 group-hover:scale-110 transition-transform ${formData.tipo === type.id ? "ring-1 ring-primary/30" : ""}`}
                                >
                                  <span
                                    className={`material-symbols-outlined text-xl ${type.color || ""}`}
                                  >
                                    {type.icon}
                                  </span>
                                </div>
