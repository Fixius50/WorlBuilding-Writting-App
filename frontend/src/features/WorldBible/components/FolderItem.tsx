  ...
  return (
    <div className={`space-y-1 select-none ${className || ""}`}>
      <div
        ...
        onContextMenu={(e) =>
          handleContextMenu(e, "folder", folder.id, folder.nombre)
        }
      ...
      >
        <span
          onClick={toggle}
          className={`material-symbols-outlined text-lg transition-transform hover:text-indigo-400 ${isOpen ? "rotate-90 text-indigo-400" : "opacity-50"}`}
        >
          chevron_right
        </span>

        <span
          className={`material-symbols-outlined text-lg ${getHierarchyVisuals(folder.tipo).color}`}
        >
      ...
      {contextMenu && (
        <div
          className="fixed monolithic-panel shadow-2xl rounded-none py-2 z-50 w-48 text-xs font-medium flex flex-col"
          style={{ top: contextMenu.y, left: contextMenu.x }}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="px-4 py-2 border-b border-foreground/10 font-bold text-foreground/60 truncate">
            {contextMenu.name}
          </div>
          {contextMenu.type === "folder" ? (
            <>
              ...
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "entidadindividual");
                  closeContextMenu();
                }}
              >
              ...
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "magia");
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm mt-0.5">
                ...
                Add Magic Archetype (Individual)
              </button>
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "entidadcolectiva");
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm mt-0.5">
              ...
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "claseprofesion");
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm mt-0.5">
                ...
                Add Class/Profession Archetype (Collective)
              </button>
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "zona");
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm mt-0.5">
              ...
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "objeto");
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm mt-0.5">
              ...
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "texto");
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm mt-0.5">
              ...
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2"
                onClick={() => {
                  onCreateEntity(folder.id, "hechizo");
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm mt-0.5">
              ...
              <button
                className="px-4 py-2 hover:bg-red-500/10 text-red-400 text-left flex items-center gap-2"
                onClick={() => {
                  onDeleteFolder(folder.id, folder.padre_id);
                  closeContextMenu();
                }}
              >
            ...
            <>
              <button
                className="px-4 py-2 hover:bg-foreground/5 text-left flex items-center gap-2 cursor-pointer"
                onClick={() => {
              ...
              <button
                className="px-4 py-2 hover:bg-red-500/10 text-red-400 text-left flex items-center gap-2"
                onClick={() => {
                  onDeleteEntity(contextMenu.id, folder.id);
                  closeContextMenu();
                }}
              >
                <span className="material-symbols-outlined text-sm">
        ...
        <div className="ml-6 pl-4 border-l border-foreground/10 space-y-1">
          ...
          {content.entities
            .filter((ent) => {
              ...
              const matchesSearch = ent.nombre
                .toLowerCase()
                .includes(searchTerm.toLowerCase());
              const matchesType =
                filterType === "ALL" || ent.tipo === filterType;
            ...
            .map((ent) => (
              <Link
                ...
                onContextMenu={(e) =>
                  handleContextMenu(e, "entity", ent.id, ent.nombre)
                }
              ...
              >
                <span className="material-symbols-outlined text-sm opacity-70 group-hover:text-indigo-400 transition-colors">
          ...
          {loaded &&
            ...
            content.entities.length === 0 && (
              <div className="px-3 py-2 text-[10px] text-foreground/60 uppercase font-black italic tracking-tighter">
                Sector Vacío
              </div>
            )}
        </div>
