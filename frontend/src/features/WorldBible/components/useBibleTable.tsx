...
export const useBibleTable = (
  ...
  } = useWorldBibleData(projectId);
  const { createEntity, bulkDelete, isCreating } =
    useWorldBibleMutations(projectId);
  ...
  const [newEntityFolderId, setNewEntityFolderId] = useState<number | null>(
    null,
  );
  ...
  const filteredData = useMemo(() => {
    ...
    return safeEntities.filter((e) => {
      ...
      if (filterType === "ENTITIES")
        return !["mapa", "timeline", "map", "linea_temporal"].includes(
          e.tipo.toLowerCase(),
        );
      if (filterType === "MAPS")
        return ["mapa", "map"].includes(e.tipo.toLowerCase());
      if (filterType === "TIMELINES")
        return ["timeline", "linea_temporal"].includes(e.tipo.toLowerCase());
      return true;
    });
