...
export interface UniversalCanvasProps {
  ...
  onDropData?: (data: string, x: number, y: number) => void;
  dropDataMimeType?: string;
...
const UniversalCanvas: React.FC<UniversalCanvasProps> = ({
  ...
  const handleDrop = (e: React.DragEvent) => {
    e.preventDefault();
    const droppedData = e.dataTransfer.getData(dropDataMimeType);
    if (droppedData && onDropData && stageRef.current) {
      const stage = stageRef.current;
      ...
      onDropData(droppedData, canvasX, canvasY);
    }
  };
