...
export const territoryEntityQueryKey = (entityId: number) =>
...
export const useTerritoryProfile = (propEntityId?: string | number) => {
  ...
  const { data: profileData, isLoading: loading } = useQuery({
    queryKey: territoryEntityQueryKey(numericEntityId),
    ...
    queryFn: async (): Promise<{
      ...
      const territoryEntity: Entidad = data!;
      const extra =
        typeof territoryEntity.contenido_json === "string"
          ? JSON.parse(territoryEntity.contenido_json)
          : territoryEntity.contenido_json || {};

      const vals = await TemplateUseCase.getEntityValues(territoryEntity.id);
      const allEntities = await EntityUseCase.getAllByProject(
        territoryEntity.project_id,
      );
      const children = allEntities.filter((e) => {
        const eExtra =
          typeof e.contenido_json === "string"
            ...
            : e.contenido_json || {};
        return (
          e.carpeta_id === territoryEntity.id ||
          eExtra.padre_id === territoryEntity.id ||
          eExtra.parent_id === territoryEntity.id
        );
      });

      return {
        entity: { ...extra, ...territoryEntity },
        attributes: vals,
        subNodes: children,
      };
    },
