...
export const collectiveEntityQueryKey = (entityId: number) =>
  ["collective-entity", entityId] as const;

/**
 * 🧠 useCollectiveProfile
...
export const useCollectiveProfile = (propEntityId?: string | number) => {
  ...
  const { data: profileData, isLoading: loading } = useQuery({
    queryKey: collectiveEntityQueryKey(numericEntityId),
    ...
    queryFn: async (): Promise<{
      ...
      const collectiveEntity: Entidad = data!;
      const extra =
        typeof collectiveEntity.contenido_json === "string"
          ? JSON.parse(collectiveEntity.contenido_json)
          : collectiveEntity.contenido_json || {};

      const vals = await TemplateUseCase.getEntityValues(collectiveEntity.id);
      const allEntities = await EntityUseCase.getAllByProject(
        collectiveEntity.project_id,
      );
      const children = allEntities.filter((e) => {
        const eExtra =
          typeof e.contenido_json === "string"
            ...
            : e.contenido_json || {};
        return (
          e.carpeta_id === collectiveEntity.id ||
          eExtra.padre_id === collectiveEntity.id ||
          eExtra.parent_id === collectiveEntity.id
        );
      });

      return {
        entity: { ...extra, ...collectiveEntity },
        attributes: vals,
        subNodes: children,
      };
    },
