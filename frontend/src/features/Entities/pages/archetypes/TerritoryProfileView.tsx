...
const TerritoryProfileView: React.FC<{ entityId?: string | number }> = ({ entityId: propEntityId }) => {
  ...
  if (!entity) return <div className="p-20 text-center text-destructive bg-background h-full font-black uppercase tracking-widest">404: TERRITORIO NO ENCONTRADA</div>;
  ...
  return (
    <div className="flex-1 bg-background flex flex-col h-full w-full animate-in fade-in duration-1000">
      ...
      <div className={`flex-1 relative ${activeTab === 'MAPA_TACTICO' ? 'overflow-hidden' : 'overflow-y-auto custom-scrollbar'}`}>
        {activeTab === 'REGISTRO' && (
