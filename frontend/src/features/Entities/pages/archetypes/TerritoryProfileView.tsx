...
const TerritoryProfileView: React.FC<{ entityId?: string | number }> = ({ entityId: propEntityId }) => {
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-0">
            {subNodes.length > 0 ? (
              ...
              <div className="p-16 text-center flex flex-col items-center gap-4">
                <span className="material-symbols-outlined text-4xl text-foreground/5">location_on</span>
  ...
  if (!entity) return <div className="p-20 text-center text-destructive bg-background h-full font-black uppercase tracking-widest">404: TERRITORIO NO ENCONTRADO</div>;
  ...
  return (
    <div className="flex-1 bg-background flex flex-col h-full w-full animate-in fade-in duration-1000">
      ...
      <div className={`flex-1 relative ${activeTab === 'MAPA_TACTICO' ? 'overflow-hidden' : 'overflow-y-auto no-scrollbar'}`}>
        {activeTab === 'REGISTRO' && (
          <main className="p-12 lg:p-24 space-y-24 max-w-6xl mx-auto w-full">
            <section className="space-y-12">
              <div className="flex flex-col items-center gap-4">
                <h3 className="text-[10px] font-black text-foreground uppercase tracking-[0.4em] border-b border-primary/40 pb-2">CRÓNICA TERRITORIAL</h3>
              </div>
        ...
        {activeTab === 'MAPA_TACTICO' && (
          <div className="w-full h-full relative bg-background flex flex-col items-center justify-center p-24">
             <div className="size-64 rounded-full border-4 border-foreground/5 flex items-center justify-center relative mb-12">
               ...
               <span className="material-symbols-outlined text-6xl text-foreground/5">map</span>
             </div>
