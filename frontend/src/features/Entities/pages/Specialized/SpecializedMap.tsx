    ...
    <div className="w-full h-full p-4 animate-fade-in">
      <MonolithicPanel className="w-full h-[600px] relative overflow-hidden bg-background/40 border-foreground/40 group">
        ...
        {pins.map(pin => (
          ...
          >
            <span className={`material-symbols-outlined text-4xl text-${pin.color}-500 drop-shadow-[0_0_15px_rgba(var(--${pin.color}-rgb),0.5)]`}>
              {pin.type}
            </span>
            <span className={`text-[10px] uppercase font-black tracking-widest text-${pin.color}-100 bg-background/50 px-2 py-1 rounded-full border border-${pin.color}-500/30 opacity-0 group-hover/pin:opacity-100 transition-opacity whitespace-nowrap`}>
              {pin.label}
            </span>
          </div>
        ))}
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <button onClick={handleAddPin} className="p-2 rounded-none sunken-panel/80 border border-foreground/10 hover:bg-foreground/10 text-foreground transition-all shadow-xl">
            <span className="material-symbols-outlined">add_location</span>
          </button>
          <button onClick={handleToggleLayers} className="p-2 rounded-none sunken-panel/80 border border-foreground/10 hover:bg-foreground/10 text-foreground transition-all shadow-xl">
