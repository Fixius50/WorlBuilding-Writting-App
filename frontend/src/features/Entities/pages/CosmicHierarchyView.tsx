...
const CosmicHierarchyView: React.FC<CosmicHierarchyViewProps> = ({ id }) => {
  const { tags, planets, handleAddTag, handleAddEntity, handleSave } =
    useCosmicHierarchyView(id);
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-4 max-w-3xl">
              <div className="flex items-center gap-3 px-3 py-1 rounded bg-orange-500/10 text-orange-400 border border-orange-500/20 inline-block">
                ...
                <span className="text-[9px] font-bold text-foreground/60 tracking-widest ml-2">
                  ID: SYS-SOL-001
                </span>
              </div>
              <p className="text-foreground/60 text-lg leading-relaxed">
                Sistema planetario principal ubicado en el Brazo de Orión, hogar
                de la única vida biológica compleja confirmada en el sector
                galáctico Alpha-9.
              </p>
            </div>
          ...
          <div className="lg:col-span-1 space-y-8">
            <section className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-auto text-[9px] font-bold text-foreground/60 px-1.5 py-0.5 rounded bg-foreground/5">
                  {tags.length} ACTIVAS
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="size-8 rounded-full border border-dashed border-foreground/40 flex items-center justify-center text-foreground/60 hover:text-foreground hover:border-foreground/40 transition-all"
                >
                  +
                </button>
              </div>
            ...
            <section className="space-y-6">
              <div className="flex items-center gap-2">
                ...
                <button className="ml-auto text-foreground/60 hover:text-foreground transition-colors">
              ...
              <div className="space-y-4">
                <Attribute
                  label="Forma / Estructura"
                  value="Heliosférica Estándar"
                  icon="hive"
                />
                <Attribute
                  label="Composición"
                  value="Hidrógeno (74%), Helio (24%)"
                  icon="experiment"
                />
            ...
            <MonolithicPanel className="h-full border-foreground/10 monolithic-panel/40 overflow-hidden flex flex-col shadow-2xl">
              <header className="p-8 border-b border-foreground/10 flex justify-center gap-12 text-center items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-foreground text-lg tracking-tight">
                      Contenido Interno (Planetas)
                    </h3>
                    <p className="text-[10px] font-medium text-foreground/60 mt-0.5">
                      Arrastra para reordenar la jerarquía orbital
                    </p>
                  </div>
                </div>
                <div className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  ...
                  {planets.length} Entidades
                </div>
              </header>

              <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-3">
                {planets.map((planet, idx) => (
                  <PlanetRow key={idx} {...planet} />
                ))}
              </div>
              ...
              <div className="p-8 bg-background/20 border-t border-foreground/10">
                <button
                  ...
                  className="w-full py-4 rounded-[1.5rem] border-2 border-dashed border-foreground/40 text-foreground/60 hover:text-foreground hover:border-primary/50 hover:bg-primary/5 transition-all flex items-center justify-center gap-3 font-black uppercase text-[10px] tracking-[0.2em]"
                >
...
const Tag: React.FC<TagProps> = ({ label, color, icon }) => (
  <div
    className={`px-4 py-1.5 rounded-none bg-${color}-500/10 text-${color}-400 border border-${color}-500/20 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-lg`}
  >
...
const Attribute: React.FC<AttributeProps> = ({ label, value, unit, icon }) => (
  <div className="space-y-2">
    <p className="text-[9px] font-black uppercase tracking-widest text-foreground/60 ml-1">
      {label}
    </p>
    <div className="w-full monolithic-panel rounded-none px-5 py-3.5 flex items-center justify-between group hover:border-foreground/40 transition-all">
      <div className="flex items-center gap-3">
        {icon && (
          <span className="material-symbols-outlined text-foreground/60 text-lg group-hover:text-primary transition-colors">
            {icon}
          </span>
        )}
...
const PlanetRow: React.FC<PlanetRowProps> = ({
  ...
  <div
    className={`group flex items-center gap-6 p-4 rounded-none transition-all cursor-pointer ${active ? "bg-primary/10 border border-primary/20 shadow-xl" : "hover:bg-foreground/5 border border-transparent"}`}
  >
    <span className="material-symbols-outlined text-foreground/60 group-hover:text-foreground/60">
      drag_indicator
    </span>
    <div
      className={`size-12 rounded-none flex items-center justify-center ${active ? "bg-primary text-foreground shadow-lg shadow-primary/30" : "monolithic-panel border border-foreground/10 text-foreground/60"}`}
    >
    ...
    <div className="flex-1">
      <div className="flex items-center gap-3">
        ...
        <span
          className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-widest ${active ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-foreground/5 text-foreground/60"}`}
        >
          {type}
        </span>
      </div>
    ...
    <div className="flex flex-col items-end gap-1">
      <div className="flex items-center gap-1.5 text-foreground/60">
        ...
        <span className="text-[10px] font-mono tracking-tight group-hover:text-foreground/60">
          {stats}
        </span>
      </div>
    </div>
    <div className="flex gap-2 ml-4">
      <button className="p-2 rounded-none text-foreground/60 hover:text-foreground hover:bg-foreground/5 transition-all">
        <span className="material-symbols-outlined text-xl">edit_note</span>
      </button>
      {active ? (
        <button className="size-8 rounded-full bg-primary text-foreground flex items-center justify-center shadow-lg">
          <span className="material-symbols-outlined text-lg">
            arrow_forward
          </span>
        </button>
