...
const CosmicHierarchyView: React.FC<CosmicHierarchyViewProps> = ({ id }) => {
  const { tags, planets, handleAddTag, handleAddEntity, handleSave } = useCosmicHierarchyView(id);
        ...
        <header className="mb-12 space-y-4">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-3">
              ...
              <Button variant="primary" icon="save" onClick={handleSave}>Guardar</Button>
            </div>
          </div>
        </header>
          ...
          <div className="lg:col-span-1 space-y-8">
            <section className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-auto text-[9px] font-bold text-foreground/60 px-1.5 py-0.5 rounded bg-foreground/5">{tags.length} ACTIVAS</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {tags.map((tag, idx) => (
                  <Tag key={idx} label={tag.label} color={tag.color} icon={tag.icon} />
                ))}
                <button onClick={handleAddTag} className="size-8 rounded-full border border-dashed border-foreground/40 flex items-center justify-center text-foreground/60 hover:text-foreground hover:border-foreground/40 transition-all">+</button>
              </div>
            </section>
          ...
          <div className="lg:col-span-2">
            <MonolithicPanel className="h-full border-foreground/10 monolithic-panel/40 overflow-hidden flex flex-col shadow-2xl">
              ...
              <div className="p-8 bg-background/20 border-t border-foreground/10">
                <button onClick={handleAddEntity} className="w-full py-4 rounded-[1.5rem] border-2 border-dashed border-foreground/40 text-foreground/60 hover:text-foreground hover:border-primary/50 hover:bg-primary/5 transition-all flex items-center justify-center gap-3 font-black uppercase text-[10px] tracking-[0.2em]">
