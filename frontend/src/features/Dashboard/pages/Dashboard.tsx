  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#0a0a0c] overflow-y-auto custom-scrollbar p-8">
      <header className="mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
        <p className="text-foreground/60 font-serif italic text-lg opacity-80">
          Panorama analítico de tu universo creativo.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        <StatCard
          title="Entidades Totales"
          value={isLoading ? "..." : stats.entityCount}
          icon="inventory_2"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
        ...
        <MonolithicPanel className="h-[400px] p-6 flex flex-col">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground/60 mb-6 px-2">
            Distribución de Entidades
          </h3>
          <div className="flex-1 min-h-0">
            {isLoading ? (
              <div className="h-full flex items-center justify-center text-foreground/60 text-xs italic">
                Cargando...
              </div>
              ...
              <ResponsivePie
                ...
                theme={{
                  ...
                  labels: {
                    text: { fontSize: 10, fontWeight: 800, fill: "#fff" },
                  },
                }}
              />
            ) : (
          ...
          <div className="flex-1 min-h-0">
            <ResponsiveBar
              ...
              theme={{
                axis: {
                  ticks: {
                    text: { fill: "#64748b", fontSize: 10, fontWeight: 600 },
                  },
      ...
      <MonolithicPanel className="p-8">
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground/60 mb-8">
          Cronología de Cambios Recientes
        </h3>
        <div className="space-y-4">
          {isLoading ? (
            <p className="text-xs text-foreground/60 italic text-center py-8">
              Cargando...
            </p>
          ) : (
            stats.recentActivity.map((e) => (
              <div
                key={e.id}
                className="flex items-center justify-between p-4 rounded-none bg-background border border-foreground/10 hover:border-foreground/40 transition-all group"
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400 px-3 py-1 rounded bg-indigo-400/10">
                  {e.tipo}
                </span>
              </div>
            ))
