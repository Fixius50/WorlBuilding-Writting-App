...
const EntityDatabase: React.FC<EntityDatabaseProps> = ({ projectId }) => {
  ...
  if (selectedEntity) {
    const img =
      selectedEntityAttrs.imageUrl ||
      selectedEntityAttrs.image ||
      selectedEntityAttrs.avatar ||
      "";
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-foreground/[0.03] border border-foreground/5 space-y-1">
              ...
              <div className="text-xs font-bold text-foreground/80 truncate">
                {folderName(selectedEntity.carpeta_id)}
              </div>
            </div>
            <div className="p-3 bg-foreground/[0.03] border border-foreground/5 space-y-1">
              <div className="text-[9px] font-black uppercase tracking-widest text-foreground/30">
            ...
            <div className="col-span-2 p-3 bg-foreground/[0.03] border border-foreground/5 space-y-1">
              ...
              <div className="text-xs font-mono text-foreground/60">
                {new Date(selectedEntity.fecha_actualizacion).toLocaleString(
                  "es-ES",
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {tags.map((tag: string, i: number) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 text-[9px] font-bold bg-primary/10 text-primary border border-primary/20 uppercase tracking-widest"
                  >
                    {tag}
                  </span>
                ))}
      ...
      <div className="shrink-0 p-4 space-y-3 border-b border-foreground/10 bg-foreground/[0.01]">
        ...
        <div className="flex gap-2">
          ...
          <div className="relative flex-1">
            ...
            <span className="material-symbols-outlined absolute right-2 top-1/2 -translate-y-1/2 text-[11px] text-foreground/40 pointer-events-none">
              expand_more
            </span>
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="divide-y divide-foreground/[0.05]">
            {filtered.map((entity) => {
              const attrs = (() => {
                try {
                  return typeof entity.contenido_json === "string"
                ...
                >
                  ...
                  <div className="size-9 shrink-0 overflow-hidden border border-foreground/10 bg-foreground/5 flex items-center justify-center">
                    {img ? (
                      ...
                      <span className="material-symbols-outlined text-sm text-foreground/30">
