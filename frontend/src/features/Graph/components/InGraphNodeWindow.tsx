...
const InGraphNodeWindow = ({
  ...
  return (
    <div
      className={`w-[260px] flex flex-col shadow-2xl border-t-4 ${categoryConfig.color} bg-foreground/5 border border-foreground/40 pointer-events-none overflow-hidden transition-all duration-300`}
    >
      {/* Header - Transparent to clicks for drag passthrough */}
      <div
        className={`p-3 border-b border-foreground/10 flex items-center justify-between pointer-events-none text-drag-handle bg-foreground/5 select-none`}
      >
        <div className="flex items-center gap-2 overflow-hidden pointer-events-none">
          <span
            className={`material-symbols-outlined text-[14px] ${categoryConfig.color.replace("border-", "text-")}`}
          >
            {categoryConfig.icon}
          </span>
          <h3
            className={`text-[10px] font-serif font-black truncate uppercase tracking-tight text-foreground`}
          >
        ...
        <div className="flex gap-1">
          <button
            onClick={onCenter}
            className="p-1 hover:bg-foreground/10 rounded-none text-foreground/60 transition-colors pointer-events-auto"
            title="Centrar Arcano"
          >
          ...
          <>
            {activeTab === "ESENCIA" && (
              <div className="space-y-4 animate-in fade-in slide-in-from-top-1 duration-300">
                <div className="space-y-1">
                  <span className="text-[8px] font-black uppercase text-foreground/60 tracking-widest">
                    Resumen Etéreo
                  </span>
                  <p className="text-xs text-foreground/60 leading-relaxed italic border-l border-foreground/40 pl-3">
                    {data.description
                      ...
                      : data.summary
                        ? String(data.summary)
                        : "Ningún cronista ha registrado detalles sobre esta entidad."}
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 gap-1.5 pt-2">
                    {attributes.map(([key, value]) => {
                      ...
                      return (
                        <div
                          key={key}
                          className="flex justify-between items-center p-2 rounded bg-background border border-foreground/10"
                        >
                          <span className="text-[8px] font-bold text-foreground/60 uppercase">
                            {key.replace(/_/g, " ")}
                          </span>
                          <span
                            className="text-[10px] text-foreground/60 truncate ml-4 font-medium"
                            title={displayValue}
                          >
                            {displayValue}
                          </span>
            ...
            {activeTab === "RELACIONES" && (
              <div className="space-y-2 animate-in fade-in slide-in-from-top-1 duration-300">
                {relations.length === 0 ? (
                  <p className="text-[10px] text-foreground/60 italic py-8 text-center">
                    Sin hilos de causalidad activos.
                  </p>
                ) : (
                  relations.map((rel) => (
                    <div
                      key={rel.id}
                      className="flex items-center justify-between p-2.5 rounded bg-background border border-foreground/10 group/rel"
                    >
