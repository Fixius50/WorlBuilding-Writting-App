...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  return (
    <>
      ...
      >
        <span
          className={`material-symbols-outlined text-[1.125rem] transition-transform duration-300 ${isOpen ? "text-primary rotate-180" : "text-indigo-500"}`}
        >
      ...
      >
        ...
        <div className="shrink-0 h-12 flex items-stretch border-b border-foreground/10 bg-foreground/[0.02] relative">
          ...
          <div className="flex items-center gap-2 px-5 border-r border-foreground/10 shrink-0">
            ...
            <span className="text-[0.65rem] font-black uppercase tracking-widest text-foreground/60">
              Panel de Control
            </span>
          </div>
          ...
          {projectName && (
            <div className="hidden md:flex items-center gap-2 px-5 border-l border-foreground/10 shrink-0 text-foreground/30">
              ...
              <span className="text-[0.6rem] font-bold truncate max-w-[10rem]">
                {projectName}
              </span>
            </div>
          )}
        ...
        <div className="flex-1 relative overflow-hidden bg-background">
          {/* BASE DE DATOS */}
          <div
            className={`absolute inset-0 transition-opacity duration-200 ${activeSection === "database" ? "opacity-100 pointer-events-auto z-10" : "opacity-0 pointer-events-none z-0"}`}
          >
          ...
          {/* NOTAS */}
          <div
            className={`absolute inset-0 transition-opacity duration-200 ${activeSection === "notes" ? "opacity-100 pointer-events-auto z-10" : "opacity-0 pointer-events-none z-0"}`}
          >
