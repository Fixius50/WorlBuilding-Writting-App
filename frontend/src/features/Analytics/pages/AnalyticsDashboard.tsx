...
const AnalyticsDashboard = () => {
  ...
  const { stats, isLoading, cards, progress } =
    useAnalyticsDashboard(projectId);
  ...
  return (
    <div className="flex-1 p-8 lg:p-12 max-w-7xl mx-auto w-full animate-in fade-in duration-700 overflow-y-auto custom-scrollbar">
      <header className="mb-12 space-y-2">
        <p className="text-sm text-foreground/40 font-bold uppercase tracking-[0.3em]">
          Métricas Reales de tu Universo
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
        ...
        <MonolithicPanel className="p-8 h-[450px] flex flex-col">
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-foreground/60 mb-8">
        ...
        <MonolithicPanel className="p-8 flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-foreground/60">
      ...
      <div className="p-8 bg-foreground/[0.02] border border-foreground/10 rounded-none flex items-center gap-8 relative overflow-hidden">
        ...
        <div className="space-y-1 relative z-10">
          ...
          <p className="text-[11px] text-foreground/60 leading-relaxed max-w-3xl">
            Has documentado un total de{" "}
            <strong className="text-foreground">
              {stats.entityCount} entidades
            </strong>{" "}
            y tejido{" "}
            <strong className="text-foreground">
              {stats.wordCount} palabras
            </strong>{" "}
            en tus crónicas. La mayor densidad de conocimiento se concentra en
            la categoría{" "}
            <strong className="text-primary italic">
              {stats.entitiesByType[0]?.label || "General"}
            </strong>
            . Sigue esculpiendo el vacío; cada entrada fortalece la coherencia
            de tu universo.
          </p>
        </div>
      </div>
