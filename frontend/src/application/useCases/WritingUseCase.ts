...
export class WritingUseCase {
  ...
  static async createNotebook(projectId: number, title: string, genre: string): Promise<Cuaderno> {
  ...
  static async updateNotebook(notebookId: number, title: string, genre: string): Promise<void> {
    const trimmedTitle = title.trim();
    if (!trimmedTitle) throw new Error("El título no puede estar vacío");
    await notebookService.update(notebookId, { titulo: trimmedTitle, genero: genre });
  }
  ...
  static async createSnapshot(pageId: number, htmlContent: string): Promise<void> {
