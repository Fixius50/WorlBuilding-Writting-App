...
import { Entidad, Carpeta, Plantilla, FolderType } from '@domain/models/database';
...
export class WorldBibleUseCase {
  ...
  static async createCategory(name: string, projectId: number, type: FolderType = 'FOLDER'): Promise<Carpeta> {
  ...
  static async quickUpdateEntity(id: number, field: string, value: any): Promise<void> {
    await entityService.update(id, { [field]: value });
  }

  /** Mueve una lista de entidades a otra carpeta de destino */
  static async moveEntitiesToFolder(entityIds: number[], folderId: number | null): Promise<void> {
    await Promise.all(entityIds.map(id => entityService.update(id, { carpeta_id: folderId })));
  }
  ...
  static async createEntityWithAttributes(
    entityData: { nombre: string; tipo: string; project_id: number; carpeta_id: number | null; descripcion?: string },
    ...
    if (entity.id && attributes.length > 0) {
      const attributePromises = attributes.map(attr => 
        entityService.addValue(entity.id!, attr.templateId, attr.value)
      );
