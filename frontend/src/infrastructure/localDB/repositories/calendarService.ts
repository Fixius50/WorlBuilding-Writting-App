...
export const calendarService = {
  async getByProject(projectId: number): Promise<Calendario[]> {
    const result =
      await sql`SELECT * FROM calendarios WHERE project_id = ${projectId} AND borrado = 0`;
    return result as Calendario[];
  },

  async create(
    calendario: Omit<Calendario, "id" | "borrado">,
  ): Promise<number> {
  ...
  async update(id: number, calendario: Partial<Calendario>): Promise<void> {
    ...
    if (calendario.nombre !== undefined) {
      fields.push("nombre = ?");
      values.push(calendario.nombre);
    }
    if (calendario.meses_json !== undefined) {
      fields.push("meses_json = ?");
      values.push(calendario.meses_json);
    }
    if (calendario.dias_semana_json !== undefined) {
      fields.push("dias_semana_json = ?");
      values.push(calendario.dias_semana_json);
    }
    if (calendario.fecha_inicio_json !== undefined) {
      fields.push("fecha_inicio_json = ?");
      values.push(calendario.fecha_inicio_json);
    }
  ...
  async delete(id: number): Promise<void> {
    ...
    emitUIRefresh({ operation: "delete", scope: "calendar", id });
  },
};
